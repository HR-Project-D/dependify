    ...
    <aside className='flex h-full w-96 flex-col justify-between border-r border-white-8 bg-gray-1'>
      <ul className='flex flex-col gap-1.5 p-4'>
        ...
        <SidebarMenuItem href='/search'>
          <IconSearch className={`${path === "/search" ? "text-white" : "text-gray-10"} w-5`} />
          Search
        </SidebarMenuItem>
        <SidebarMenuItem href='/data-sources'>
          <IconDatabase className={`${path === "/data-sources" ? "text-white" : "text-gray-10"} w-5`} />
          Data Sources
        </SidebarMenuItem>
        <SidebarMenuItem href='/alerts'>
          <IconNotification className={`${path === "/alerts" ? "text-white" : "text-gray-10"} w-5`} />
          Alerts
        </SidebarMenuItem>
      </ul>
  ...
  return (
    <Link href={href}>
      <li className={`relative flex w-52 px-4 py-2.5 text-sm font-medium`}>
        <span className={`${isActive ? "text-white" : "text-white-48"} z-10 flex items-center gap-3 transition-all duration-300`}>
          {children}
        </span>
        {isActive && (
          <motion.div layoutId='sidebar-active-item' className='absolute left-0 top-0 h-full w-full rounded-lg bg-white-8'></motion.div>
        )}
      </li>
    </Link>
  );
